...
ObstacleDetector::ObstacleDetector(ros::NodeHandle nh) : nh_(nh), cnt_(0)
{
  // Get params
  ros::NodeHandle nhp("~");
  ...
  // Sonar subscription
  sub_ = nh_.subscribe("/sonar", 20, &ObstacleDetector::sonarCb, this);
}


void ObstacleDetector::sonarCb(const miniking_ros::AcousticBeamConstPtr beam)
{
  ...
  // ROS_INFO_STREAM("ANGLE IDX: " << angle_idx << " ANGLE : " << beam->angle << " LEFT_LIMIT: " << beam->left_limit << " RIGHT_LIMIT: " << beam->right_limit << "  RANGE:"<<ls_.ranges[angle_idx] << " MAX_INTENSITY:"<<ls_.intensities[angle_idx]);
  if ((angle_idx == 0 || angle_idx == (ls_.ranges.size() - 1) )) {
    if (!first_execution_) {
      ls_pub_.publish(ls_);
      for (size_t i = 0; i < ls_.ranges.size(); i++) {
        ls_.ranges[i] = ls_.range_max;
        ls_.intensities[i] = 0;
      }

    } else if (first_execution_){
